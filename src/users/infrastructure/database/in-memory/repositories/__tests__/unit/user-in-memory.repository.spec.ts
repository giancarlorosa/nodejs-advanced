...
describe('UserInMemoryRepository unit tests', () => {
  ...
  it('shoud throw error when not found - findByEmail method', async () => {
    await expect(sut.findByEmail('a@a.com')).rejects.toThrow(
      new NotFoundError('Entity not found using email a@a.com'),
    );
  });

  it('shoud find an entity by email - findByEmail method', async () => {
  ...
  it('shoud throw error when not found - emailExists method', async () => {
    const entity = new UserEntity(UserDataBuilder({}));
    await sut.insert(entity);
  ...
  it('shoud find an entity by email - emailExists method', async () => {
    expect.assertions(0);
    await sut.emailExists('a@a.com');
  });

  it('shoud not filter items when filter object is null', async () => {
    const entity = new UserEntity(UserDataBuilder({}));
    await sut.insert(entity);
  ...
  it('shoud filter name field using filter param', async () => {
    const items = [
  ...
  it('shoud sort by createdAt when sort param is null', async () => {
  ...
  it('shoud sort by name field', async () => {
    const items = [
